...
for i in range(0,n_x+2):
    ...
    data_Y_1.append(F(data_X[i]))
# Зберігаємо точки X та початкові умови
data = pd.DataFrame({'x': data_X, 'y=' + str(y_0): data_Y_0, str(y_0+delta_y): data_Y_1})
# Обчислюємо значення у наступних точках Y та зберігаємо результати
for j in range(2, n_y+2):
    # Лівий кінець відрізка 
    data_Y_i = [2*data.iloc[0,j]-data.iloc[0,j-1]+(39*delta_y**2)*(data.iloc[2,j]-2*data.iloc[1,j]+data.iloc[0,j])/(19*delta_x**2)]
    # Внутрішні точки відрізка
    for i in range(1,n_x+1):
        data_Y_i.append(2*data.iloc[i,j]-data.iloc[i,j-1]+(39*delta_y**2)*(data.iloc[i+1,j]-2*data.iloc[i,j]+data.iloc[i-1,j])/(19*delta_x**2))
    # Правий кінець відрізка
    data_Y_i.append(2*data.iloc[n_x+1,j]-data.iloc[n_x+1,j-1]+(39*delta_y**2)*(data.iloc[n_x+1,j]-2*data.iloc[n_x,j]+data.iloc[n_x-1,j])/(19*delta_x**2))
...
data.to_excel('hyperbolic_number.xlsx')
...
data_exact.to_excel('hyperbolic_exact.xlsx')
...
data_errors.to_excel('hyperbolic_errors.xlsx')